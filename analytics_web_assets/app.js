...
function render(){
 ...
 if(state.view==='weekly')html=`<div class="section-head"><div><h2>Weekly job review</h2><p>${displayDate(d.filters.start)} through ${displayDate(d.filters.end)}</p></div><div class="actions"><button data-save-snapshot ${state.snapshot?'disabled':''}>Save review snapshot</button><select id="snapshot-list" aria-label="Saved review snapshots"><option value="">Saved snapshots on this PC</option>${(d.review_store?.snapshots||[]).map((s,i)=>`<option value="${i}">${displayDate(s.generated_at)} · ${s.generated_at.slice(11,16)}</option>`).join('')}</select></div></div><div class="metrics">${card('Reviewed',reviewed.map(r=>r.id))}${card('Still to review',d.rows.filter(r=>!reviewFor(r)).map(r=>r.id))}${card('Follow-ups required',corrections.map(r=>r.id))}</div><p>Review notes and snapshots are stored on this PC, scoped to this workspace. They are not yet shared between office users.</p>`;
 if(state.view==='jobs'){state.ids=d.rows.filter(r=>!reviewFor(r)).map(r=>r.id);html='<h2>Jobs to review</h2><p>Open a job to inspect its log and requirements, then record the review.</p>';}
 if(state.view==='corrections'){state.ids=corrections.map(r=>r.id);html=`<h2>Corrections and follow-ups</h2><div class="panel">${corrections.length?corrections.map(r=>{const v=reviewFor(r);return `<button class="correction" data-review="${esc(r.id)}"><strong>${esc(r.name)}</strong><span>${esc(v.note)}</span><small>${esc(v.owner)} · Due ${displayDate(v.due)}</small></button>`;}).join(''):'No open follow-ups recorded for this review period.'}</div>`;}
