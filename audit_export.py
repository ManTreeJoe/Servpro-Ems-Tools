...
# ── Job-picker window ────────────────────────────────────────────────
# The Tk dialog moved to audit_export_dialog so this module stays pure
# logic (it is imported by the web panels). Re-exported lazily through
# __getattr__ rather than a top-level import, because a top-level one
# would (a) re-introduce the Tk dependency this move removes and (b)
# be a circular import — the dialog imports _build_pdf from here.
def __getattr__(name):
    if name == "open_export_window":
        from audit_export_dialog import open_export_window
        return open_export_window
